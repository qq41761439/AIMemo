...
- 任务管理：添加任务时使用单个“任务内容”输入框，第一行显示在任务列表中；支持勾选完成、删除任务，编辑任务时可调整创建时间和完成时间；添加/保存按钮紧跟任务表单，删除前会确认，删除后可撤销。
...
- 模型服务：总结页可在应用内配置自定义 OpenAI-compatible 模型服务，并在“模型”参数中显示当前模型名；官方托管模型入口暂未开放。
