...
当前桌面版以本地 SQLite 为主；后端提供邮箱验证码登录、微信小程序登录、云端任务、云端总结历史、免费额度和服务端模型代理能力。多端账号、桌面端与微信小程序数据同步、云端数据模型和阶段计划见 [产品与技术规划](docs/product-and-technology-plan.md)。后续 iOS/Android 移动 App 优先采用原生实现，App 产品、页面和视觉以 [移动端产品文档](docs/product-document-app.md) 为准。
...
- iOS/Android 旧 Flutter 工程已接入；新移动 App 优先按原生 iOS/Android 实现，产品结构以移动端产品文档为准。
- 原生 Android 已迁移到移动端产品文档定义的 Compose 页面结构，并按 `assets/prototypes/` 对齐登录、Tasks、Task 编辑、Summary 主入口、Summary 生成、Summary 结果、Summary 历史、Profile 和 Settings 页面。
...
- 按移动端产品文档重构原生 Android/iOS App，并补齐所需后端能力。
...
原生 Android 工程使用 Kotlin、Jetpack Compose、Compose Navigation 和 Material 3。Debug 构建默认连接 Android 模拟器可访问的宿主机地址 `http://10.0.2.2:8787`，并通过 Debug 专用网络安全配置允许本地 HTTP；Release 构建默认连接 `https://aimemo-backend.onrender.com`。App 产品、页面和视觉以 [移动端产品文档](docs/product-document-app.md) 与 `assets/prototypes/` 为准；当前登录/注册页沿用现有邮箱验证码能力。
