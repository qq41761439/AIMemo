...
当前桌面版以本地 SQLite 为主；后端提供邮箱验证码登录、微信小程序登录、云端任务、云端总结历史、免费额度和服务端模型代理能力。多端账号、桌面端与微信小程序数据同步、云端数据模型和阶段计划见 [产品与技术规划](docs/product-and-technology-plan.md)。后续 iOS/Android 移动端优先采用原生实现，设计规范见 [iOS/Android 设计规范](docs/mobile-design-guidelines.md)。
...
- iOS/Android 旧 Flutter 工程已接入；新移动端优先按原生 iOS/Android 设计规范重建云端账号版，不再沿用窄屏底部四栏导航。
- 原生 Android V1 已按移动端设计 token 调整任务、总结、历史和“我的”页的 Compose UI：顶部“任务 / 总结”切换、图标操作、轻量任务卡片、聊天式快速输入栏和紧凑总结配置区。
- 原生 Android 任务页按开始时间和完成状态分为“进行中 / 即将开始 / 已完成”，前两组可折叠，已完成任务标题弱化并显示删除线。
...
- 新安装客户端默认把官方托管后端指向 `https://aimemo-backend.onrender.com`；本机联调时可手动改回 `http://127.0.0.1:8787`。
...
- 基于后端 API 实现原生 Android/iOS 和微信小程序任务 + 总结体验。
...
- [iOS/Android 设计规范](docs/mobile-design-guidelines.md)：原生移动端设计 token、页面结构、平台适配和验收清单。
