...
- 主界面采用桌面工具布局：左侧为 AIMemo 标识、任务列表和标签过滤，右侧为添加任务、生成总结和历史记录；模板配置从生成总结区域进入。
...
- 周期总结：支持日报、周报、月报、年报和自定义总结，可用日/周/月/年/自定义日期区间收集任务。
- 提示词模板：在生成总结区域点击“配置模板”即可编辑，每个周期可单独配置模板，支持 `{tasks}`、`{period}`、`{period_days}`、`{tags}`；日报和周报默认只写已完成和下步计划，月报和年报默认包含更多复盘结构。
...
已经保存过的自定义模板会保留用户版本。在生成总结区域点击“配置模板”，再点击“恢复默认”，即可应用当前周期的新版默认模板。默认模板会按日报、周报、月报、年报和自定义总结分别提供不同结构；自定义总结会用 `{period_days}` 判断区间是否超过 7 天。
