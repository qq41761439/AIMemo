...
- 移动端标签筛选、任务卡片标签和记录表单候选标签使用紧凑展示；任务卡片标签显示在标题前，避免标签过多时撑乱页面。
- 移动端记录表单支持在短屏和键盘场景下滚动填写任务内容、标签和提交按钮，点击空白处会收起键盘。
- Flutter 移动端 Tasks 页面按 Upcoming、Active、Completed 分组展示；Upcoming 默认收起，Active 和 Completed 默认展开，点击分组标题可展开或收起。
...
- 移动 App 顶部栏、分区标题、任务行和首屏间距已按 390 × 844 基准屏收紧；Task 页面偏好轻量独立卡片、低阴影和不超过 52pt 的单行视觉高度。
- 移动端任务编辑页已将 Title 和 Notes 合并为 Task 多行输入，采用无区域外框的轻量行内表单布局。
...
- 后端已支持 Resend API 和 SMTP 邮件发送；Render 免费档建议优先配置 `RESEND_*`，否则邮箱验证码仍会只打印在后端日志。开发和联调时也可以直接输入 `1234` 作为万能验证码登录，邮件投递失败不会阻断验证码校验。
