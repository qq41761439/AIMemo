...
当前桌面版仍以本地 SQLite 为主；自定义模型模式下，客户端会直接请求用户填写的模型服务。新增的后端服务用于后续接入“普通用户不用 Key”的官方托管模型、多端账号、云端任务、云端总结历史和免费额度。
...
- 桌面客户端尚未接入后端登录和官方托管模型，当前“官方托管模型”入口仍会提示未开放。

下一步建议：

- 将桌面端“官方托管模型”模式接入 `backend/` 的邮箱登录、额度和 `/summaries/generate`。
...
- 模型服务：总结页可在应用内配置自定义 OpenAI-compatible 模型服务，并在“模型”参数中显示当前模型名；官方托管模型的后端 API 已具备基础能力，客户端登录接入仍待完成。
...
- 官方托管模式接入后，任务、标签、额度和云端总结历史会保存在 AIMemo 后端；云端历史保存总结输出和元数据，不保存完整 prompt 快照。
